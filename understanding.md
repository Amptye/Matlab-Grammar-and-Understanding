...
也可通过判断数据结构 **<font color='red'>维度扩张的方向</font>**，选择最优的 **坐标中数字变化发生的方向**（也可认为是 **<font color='red'>维度升高的方向</font>**，此时可认为**坐标中数字变化**都是 **从低到高** 发生的），从而决定是 **行优先** 还是 **列优先**，例如：
- 当数据结构是 **多维矩阵/张量** 时，**<font color='red'>维度向右扩张</font>**，采用 **列优先**（如 Matlab）
- 当数据结构是 **多维数组/z轴坐标写在左边的多维矩阵** 时，**<font color='red'>维度向左扩张</font>**，采用 **列优先**（如 NumPy）
